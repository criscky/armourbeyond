...
public class Injector2Container extends Container {
    private final IInventory inventory;
    private IIntArray fields;
    protected final Injector2TileEntity tileEntity;


    public Injector2Container(int id, PlayerInventory playerInventory, PacketBuffer buffer) {
    ...
    public Injector2Container(int id, PlayerInventory playerInventory, Injector2TileEntity tileentity/*IInventory inventory, IIntArray fields*/) {
        super(ModContainerTypes.INJECTOR2.get(), id);
    ...
    public int getProgressArrowScale() {
        ...
        if (progress > 0) {
            return (int) Math.floor(progress * 24 / Injector2TileEntity.WORK_TIME);
        }
        return 0;
    }
    ...
    private static Injector2TileEntity getTileEntity(final PlayerInventory playerInv, final PacketBuffer data) {
        Objects.requireNonNull(playerInv, "Player Inventory cannot be null.");
        Objects.requireNonNull(data, "Packet Buffer cannot be null.");
        final TileEntity te = playerInv.player.level.getBlockEntity(data.readBlockPos());
        if (te instanceof Injector2TileEntity) {
            return (Injector2TileEntity) te;
        }
